...
public class BoardView extends AppCompatImageView {
  ...
  protected  void onDraw( Canvas canvas) {
    ...
    for( int y = 0; y < 11; y++ ) {
      for( int x = 0; x < 11; x++ ) {
        BoardSetup.defendingGrid[x][y].setTopLeft( new Point( x * cellWidth, y * cellWidth ) );
        BoardSetup.defendingGrid[x][y].setBottomRight( new Point( ( x + 1 ) * cellWidth, ( y + 1 ) * cellWidth ) );
      }
    }
    ...
    for( int y = 0; y < 11; y++ ) {
      for( int x = 0; x < 11; x++ ) {
        if( BoardSetup.defendingGrid[x][y].getHasShip() ) {
          //Draw S in the Cell
          drawcell( "S", x, y, center, canvas );
        } else if( BoardSetup.defendingGrid[x][y].getHit() ) {
          // Draw an * in the cell
          drawcell( "*", x, y, center, canvas );
        } else if( BoardSetup.defendingGrid[x][y].getMiss() ) {
          // Draw an - in the cell
          drawcell( "-", x, y, center, canvas );
        } else if( BoardSetup.defendingGrid[x][y].getWaiting() ) {
  ...
  void drawcell( String contents, int x, int y, float center, Canvas canvas){
    canvas.drawText( contents, BoardSetup.defendingGrid[x][y].getTopLeft().x + center, BoardSetup.defendingGrid[x][y].getBottomRight().y - 12, paint );
  }
}
